...
- **Summative Generation Agent** (`MODEL_LIGHT`) -- generates the summative assessment (multi-step capture task + rubric + exemplar + learner-facing `courseIntro` and `summaryForLearner`) from all course learning objectives
...
- **Summative Assessment Agent** (`MODEL_HEAVY` + vision) -- scores multiple screenshots against rubric criteria; produces a `summaryForLearner` (concise plain-language read) plus detailed per-criterion scores; enforces ratchet rule (scores only go up)
...
- **Activity Creation Agent** (`MODEL_LIGHT`) -- generates one formative activity at a time, targeting specific rubric criteria; receives summative context (exemplar, task, rubric) so activities build toward the exemplar
...
1. **Summative setup**: The Summative Generation Agent creates a multi-step capture task, rubric (one criterion per objective group, each with 4 mastery levels), exemplar, and learner-facing messages (`courseIntro` explaining the process, `summaryForLearner` introducing what they'll build and what mastery looks like) from all course learning objectives.
2. **Rubric review**: The learner sees the `courseIntro` and `summaryForLearner`, then reviews the rubric/exemplar in a conversation with the Summative Rubric Review Agent. They can request changes (which triggers regeneration) or skip.
3. **Baseline attempt**: The learner attempts the summative — each step produces a screenshot capture. The Summative Assessment Agent scores all screenshots against the rubric, producing per-criterion scores and a `summaryForLearner` (concise plain-language read of how it went). The detailed breakdown is available through Q&A.
