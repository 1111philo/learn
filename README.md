...
- **AI generation** — A course describer agent runs first to establish a narrative arc and pre-set lesson titles, then each objective gets a lesson, practice activity, and scoring rubric — all generated by Claude via PydanticAI agents
...
- **Activity submission** — Write a response, get AI-scored feedback (rationale, strengths, improvements, tips) with a modal-based UI; feedback is written in second person ("you did X", "you should try Y")
...
- **Sidebar navigation** — Each lesson shows its activity as a sub-link with live status (not started / attempted / passed); clicking the activity link navigates directly back to the activity without going through the lesson first
...
backend/           Python (FastAPI + SQLAlchemy + PydanticAI)
  src/app/
    ...
    schemas/       Pydantic request/response models (incl. CourseDescriptionOutput)
